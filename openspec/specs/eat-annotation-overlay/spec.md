# eat-annotation-overlay Specification

## Purpose

How transferred (EAT) annotation values are carried, displayed and filtered alongside the curated ones they supplement. Covers the `__pred_value` / `__pred_confidence` / `__pred_source` side-channel, the reliability threshold, legend and tooltip treatment, and the guarantee that an overlay never mutates the curated column it sits beside.
