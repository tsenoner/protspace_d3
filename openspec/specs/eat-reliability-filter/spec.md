# eat-reliability-filter Specification

## Purpose

How the legend's EAT reliability control and the Filter query builder express and share one
filter. The control offers three directions — bound below, bound above, keep a band — and both
surfaces read and write the same condition, so a change made in either is visible in the other.

This capability owns the control's side of that contract: which condition counts as the control's,
how the control's position is read back out of the query, and how the two directions stay in step
without ping-ponging. How each position is _encoded_ as a query condition belongs to
