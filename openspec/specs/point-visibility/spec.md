# point-visibility Specification

## Purpose

The single model that decides what a plotted point looks like and whether it can be interacted with. It resolves hidden state, selection fading and the "Other" bucket into one numeric opacity, which is also the interactivity test — so display and hit-testing cannot disagree, and no site re-derives visibility for itself.
