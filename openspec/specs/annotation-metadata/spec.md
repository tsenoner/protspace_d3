...
The single registry describing every annotation column ProtSpace ships: its display label, provenance and whether it is a prediction. It is the source both the UI and the generated documentation page read, so a column is described in one place rather than at each display site.
