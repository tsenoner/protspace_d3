...
If your H5 keys are not valid UniProt accessions (e.g., `NCBI|...`, custom IDs), other accession-dependent annotations will be empty. Sequence-dependent annotations and the missing-length fallback can still work if you provide the original FASTA file with `-f`.
...
| `length`                  | Sequence length (UniProt-preferred; FASTA fallback when missing) | `393`                                                          |
