...
function convertBundleFormatData(rows: Rows, columnNames: string[]): VisualizationData {
  const proteinIdCol =
    findColumn(columnNames, ["identifier", "protein_id", "id", "protein", "uniprot"]) || columnNames[0];
...
function convertLegacyFormatData(rows: Rows, columnNames: string[]): VisualizationData {
  const proteinIdCol = findColumn(columnNames, ["identifier", "protein_id", "id", "protein", "uniprot"]) || columnNames[0];
  if (!proteinIdCol) {
  ...
  if (projectionPairs.length === 0) {
    ...
    if (numericColumns.length === 0) {
      throw new Error(`No projection coordinate pairs found. Available columns: ${columnNames.join(", ")}`);
    }
  }
  ...
  const projections = projectionPairs.map((pair) => {
    ...
    return { name: pair.name, data: projectionData } as VisualizationData["projections"][number];
  });
...
export function findProjectionPairs(
  ...
  for (const col of columnNames) {
    ...
    if (
      ...
      lower.includes("id") ||
      (!lower.includes("_x") && !lower.includes("_y") && !lower.includes("1") && !lower.includes("2"))
    ) {
      continue;
    }
    let projectionName = "";
  ...
  for (const [name, group] of groups.entries()) {
    if (group.x && group.y) {
      pairs.push({ name: formatProjectionName(name), xCol: group.x, yCol: group.y });
    }
  }
...
): Promise<{ features: Record<string, Feature>; feature_data: Record<string, number[]> }> {
  const allIdColumns = new Set([
    "projection_name",
    "x",
    "y",
