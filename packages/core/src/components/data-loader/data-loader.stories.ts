...
export const Default: Story = {
  ...
  render: (args) => html`
    <protspace-data-loader
      ...
      @data-loaded=${(e: CustomEvent) => {
        ...
        message.textContent = `Loaded ${
          e.detail.data.protein_ids.length
        } proteins with ${Object.keys(e.detail.data.features).length} features`;
    ...
    >
      ...
      <p>
        <strong>Expected columns:</strong> protein_id, x/y coordinates,
...
Projections: ${data.projections.length} (${data.projections
            .map((p: any) => p.name)
            .join(", ")})
Features: ${Object.keys(data.features).length} (${Object.keys(
            data.features
          ).join(", ")})
...
${Object.entries(data.features)
  ...
  .join("\n")}
    ...
    <div
      style="margin-top: 1rem; padding: 1rem; background: #e3f2fd; border-radius: 4px;"
    >
      ...
      <ul>
        ...
        <li>
          <strong>3 categorical features:</strong> function, organism, family
        </li>
      </ul>
      <p>
        The data demonstrates automatic column detection and feature extraction.
      </p>
    </div>
  `,
};
...
export const CustomColumnMapping: Story = {
  ...
  render: (args) => html`
    ...
    <div
      style="margin-top: 1rem; padding: 1rem; background: #fff3e0; border-radius: 4px;"
    >
...
export const LoadingStates: Story = {
  render: () => html`
    ...
    >
      <p>
        <strong>Note:</strong> The loading and error states shown here are
        simulated for visual reference. In actual usage, these states are
        automatically managed during file loading operations.
      </p>
    </div>
  `,
};
...
export const DropZoneOnly: Story = {
  ...
  render: (args) => html`
    ...
    <div
      style="margin-top: 1rem; padding: 1rem; background: #e8f5e8; border-radius: 4px;"
    >
      <h4>Drag & Drop Demo:</h4>
      <p>This component is optimized for drag and drop workflow:</p>
      <ul>
        <li>
          Drag an Arrow file (.arrow, .parquet, .feather) from your file system
        </li>
        <li>Drop it onto the dashed border area</li>
        <li>Watch automatic parsing and data extraction</li>
      </ul>
      <p>
        <strong>Tip:</strong> You can also click to browse files if drag & drop
        isn't available.
      </p>
    </div>
  `,
